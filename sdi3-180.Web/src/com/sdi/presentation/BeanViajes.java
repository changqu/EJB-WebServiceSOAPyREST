...
public class BeanViajes implements Serializable{
	...
	public String viajesActivos() { 
		trips=Factories.services.getPublicoService().viajesActivosPublico();
		return "exito";
	}
	...
	public void viajeDetalle(Trip t){
		...
		promotor=Factories.services.getRegistradoService().obtenerPromotor(t);
		participantes=Factories.services.getRegistradoService().obtenerParticipantes(t);
		participantesExcluidos=Factories.services.getRegistradoService().obtenerParticipantesExcluidos(t);
		solicitantes=Factories.services.getRegistradoService().obtenerSolicitantes(t);
		relacion=Factories.services.getRegistradoService().obtenerRelacion(u, t);
	}
	...
	public void solicitarPlaza(){
		User u=(User) FacesContext.getCurrentInstance()
				.getExternalContext().getSessionMap().get("LOGGEDIN_USER");
		Factories.services.getRegistradoService().solicitarPlaza(u, trip);
		inicializarViajes();
	}
	...
	public void obtenerMisViajes(){
		User u=(User) FacesContext.getCurrentInstance()
				.getExternalContext().getSessionMap().get("LOGGEDIN_USER");
		misViajes=Factories.services.getRegistradoService().misViajes(u);
	}
	
	public void obtenerViajesRelacionados(){
		User u=(User) FacesContext.getCurrentInstance()
				.getExternalContext().getSessionMap().get("LOGGEDIN_USER");
		viajesRelacionados=Factories.services.getRegistradoService().viajesRelacionados(u);
	}
	
	public void inicializarInvolucrados(){
		participantes=Factories.services.getRegistradoService().obtenerParticipantes(trip);
		participantesExcluidos=Factories.services.getRegistradoService().obtenerParticipantesExcluidos(trip);
		solicitantes=Factories.services.getRegistradoService().obtenerSolicitantes(trip);
	}
	
	public void confirmarPasajero(User userSeleccionado){ //esto no cambia mi viaje ni viajesRelacionados
		if(Factories.services.getRegistradoService().confirmarPasajero(trip, userSeleccionado)){
	...
	public void excluirPasajero(User userSeleccionado){ //esto no cambia mi viaje ni viajesRelacionados
		Factories.services.getRegistradoService().excluirPasajero(trip, userSeleccionado);
	...
	public void cancelarSolicitud(){
		User u=(User) FacesContext.getCurrentInstance()
				.getExternalContext().getSessionMap().get("LOGGEDIN_USER");
		Factories.services.getRegistradoService().cancelarSolicitud(u, trip);
		inicializarViajes();
	}
	...
	public void cancelarViajes(){
		...
		for(Trip t: viajesCancelados.keySet())
			if(viajesCancelados.get(t))//si es true, el viaje es elegido
				trips.add(t);
		Factories.services.getRegistradoService().cancelarViaje(trips);
	}
